...
peer 4.208.40.177
address 4.208.40.177
pre-shared-key changeme
peer 4.208.40.195
address 4.208.40.195
pre-shared-key changeme
!
crypto ikev2 profile AZURE-IKE-PROPOSAL
match address local 10.30.1.9
match identity remote address 4.208.40.177 255.255.255.255
match identity remote address 4.208.40.195 255.255.255.255
...
tunnel destination 4.208.40.177
...
ip tcp adjust-mss 1350
tunnel source 10.30.1.9
tunnel destination 4.208.40.195
tunnel protection ipsec profile AZURE-IPSEC-PROFILE
!
interface Loopback0
